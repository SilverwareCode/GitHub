...
public class StartupOWIN
{
    public void Configuration(IAppBuilder app)
    {
        ...
        int expTimeInHours = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["AuthCookieExpiration"]);
        ...
        app.UseCookieAuthentication(new CookieAuthenticationOptions
        {
            ...
            ExpireTimeSpan = TimeSpan.FromHours(expTimeInHours)
        });
    }
...
namespace WebSystem
{
    public class Identity
    {
        ...
        {
            //funkce zjistuje, jestli je prave prihaseny uzivatel
            //v dane roli (roleName)
            if (IsUserInRole(GetCurrentUser(), roleName))
            {
                return true;
            }
            else
        ...
        public static string GetCurrentUserIdentityId()
        {
            ...
            if (page.User.Identity.IsAuthenticated)
            {
                ...
                if (userid == null)
                {
                    retVal = "Error: userId is equal null";
                }
                else
                {
                    retVal = userid.Id;
        ...
        {
            //funkce vraci jmeno uzivatele userName
            string id = "";
            ...
            try
            {
                id = manager.FindByName(userName).Id;
            }
            catch (Exception)
            {
                id = "";
        ...
        public static string GetIdentityNameById(string id)
        {
            ...
            return manager.FindById(id).UserName;
        }
        ...
        public static bool CreateIdentityUser(string userName, string password)
        {
            ...
            if (result.Succeeded)
            {
                //Debug.WriteLine(String.Format("User {0} has been successfully registered!", user.Id));
                retVal = true;
                //login new user
